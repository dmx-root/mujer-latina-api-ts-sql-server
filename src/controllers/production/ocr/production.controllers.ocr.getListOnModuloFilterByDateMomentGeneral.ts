...
export const getListOnModuloFilterByDateMomentGeneral:(req:Request,res:Response) => Promise<any> = async (req:Request,res:Response) => {

    const { page, pageSize, tipoReferencia, filtro, moduloId } = req.query;

    const schema = yup.object().shape({
        page:yup.number().max(50).min(1),
        pageSize:yup.number().max(50).min(1),
        tipoReferencia: yup.string().required().max(2).min(2),
    ...
    try {
        ...
        const response : DbResponse = await db.execute('sp_gestion_ml_db_reportes_solicitud_ocr_lista_filtrada_modulo_todos', params);
