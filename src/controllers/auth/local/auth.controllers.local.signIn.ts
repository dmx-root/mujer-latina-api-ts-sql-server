...
export const signIn : ( req:Request, res:Response ) => Promise < ApiResponse | ApiDataResponse | any> =  async (req:Request, res:Response) => {
    const {
        documentoId,
        clave,
        perfil,
    ...
    try {
        ...
        if( response.statusCode === 0){
            const apiResponse : ApiResponse = {
                statusCode: 0,
                message: response.message || 'No se pudo obtener mensajes'
            }
            return res.status(404).json(apiResponse);
        }
