...
interface ConnectionDBInterface{
    execute:() => Promise<DbResponse>
}

export class SqlServerConnection implements ConnectionDBInterface {

    private config: sql.config;
    private procedure: string;
    private parameters: dbParameters[];

    constructor({ procedure, parameters }: { procedure: string, parameters?: Array<dbParameters> }) {
    ...
    async execute(): Promise<any> {
        return new Promise(async (resolve: (cont: DbResponse) => void, reject: (cont: DbResponse) => void) => {
            ...
            try {
                ...
                const { returnValue, recordset } = await query.execute(this.procedure);
